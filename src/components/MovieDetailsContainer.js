...
import { TMDP_API_OPTIONS, TMDP_IMAGE_BASE_URL_DASHBOARD_SIZE } from '../utils/constants'
  ...
  return (
    <div className="p-6 h-full">
      <div className="flex gap-20 divide-teal-400 divide-x-4 ">
        { videoId ? <VideoPlayContainer videoId={videoId} />: <img className='w-3/5' src={TMDP_IMAGE_BASE_URL_DASHBOARD_SIZE+moviePoster} /> }
        <div className={"px-4 text-center w-2/5 "+(videoId?"":"pt-8")}>
          <p className="text-6xl text-white font-mono my-8">{movieName}</p>
          {videoId && <button className="bg-teal-400 text-black font-semibold text-2xl px-4 py-2 w-3/5 my-1" onClick={()=> setVideoId(null) } >Poster</button> }
          <button
